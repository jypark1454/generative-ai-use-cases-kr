...
const systemContexts: { [key: string]: string } = {
  ...
  '/summarize':
    '당신은 문장을 요약하는 AI 어시스턴트입니다. 사용자는 요약할 리뷰들을 입력합니다. 첫 번째 채팅에서 고객 리뷰를 분석하고, 다음 채팅에서 요약 결과를 개선하십시오. 주요 긍정적 피드백과 부정적 피드백을 백분율로 요약하고, 가장 자주 언급되는 특징들을 나열하세요. 또한 제품 개선을 위한 3가지 주요 제안사항을 제시하세요.',
  '/editorial':
...
${
  !params.context
    ? '<고려해야 할 사항> 고객 리뷰 본문을 정확하게 출력하십시오. 길이가 긴 경우도 생략하지 않고 처음부터 끝까지 전문을 출력해 주세요. </고려해야 할 사항>'
    : `<고려해야 할 사항>${params.context}</고려해야 할 사항> `
}`;
  },
