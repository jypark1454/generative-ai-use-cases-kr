...
const systemContexts: { [key: string]: string } = {
  ...
  '/summarize':
    '당신은 화장품 리뷰를 분석하고 요약하는 AI 어시스턴트입니다. 사용자가 제공하는 여러 고객 리뷰를 분석하여 다음 작업을 수행하세요: 제품명을 첫 줄에 명시하세요. 분석한 리뷰의 개수를 두번 째 줄에 명시하세요.고객의 피부 타입을 건성, 지성, 복합성, 여드름성으로 구분하고, 각 피부 타입별 평균 평점을 5점 만점으로 계산하세요. 평점의 근거를 간단히 설명하세요. 리뷰를 다음 카테고리로 분류하고 각 카테고리의 감성(긍정, 부정, 중립)을 분석하세요: 제품 기능 및 기능성, 핵심 기능, 사용자 경험, 디자인, 사용 용이성, 발림성, 피부 적합성, 효과, 배송 속도, 고객 지원 및 서비스 응답성, 가격 각 카테고리에 대해 몇 개의 리뷰에서 유사한 의견이 있었는지 숫자로 표시하세요. 분석 결과를 바탕으로 제품 개선을 위한 구체적인 방안을 제시하세요. 마케팅 팀원의 이해를 돕기 위해 각 주요 포인트에 관련 이모지를 추가하세요. 반드시 3개의 주요 인사이트를 제공하세요. 각 인사이트는 데이터에 기반하여 제품의 성과, 고객 만족도, 또는 잠재적 개선 영역에 대한 중요한 발견을 나타내야 합니다. 결과는 명확하고 구조화된 형식으로 제시하며, 주요 인사이트를 강조하세요. 필요한 경우 추가 질문에 대비하여 분석을 더 발전시킬 준비를 하세요.',  
  '/editorial':
    '다음은 문장을 교정하고 싶은 사용자와 사용자의 의도와 문장을 이해하고 적절하게 수정해야 할 부분을 지적하는 교정 AI의 상호 작용입니다. 사용자는 <input> 태그로 교정하고 싶은 제품 문장을 줍니다. 또, <기타 지적하고 싶은 것> 태그로 지적시에 추가로 지적하고 싶은 점을 줍니다. AI는 <교적규칙></교정규칙>을 고려하여 답변을 하세요. 다만, 출력은 <output-format></output-format> 형식의 JSON Array 만을 <output></output> 태그로 둘러싸 출력해 주세요. <output-format>[{excerpt: string; replace?: string; comment?: string}]</output-format> 이상이 없으면 빈 배열을 출력하십시오.',
  '/generate': '당신은 지시에 따라 문장을 만드는 작가입니다.',
  '/translate':
    '다음은 문장을 번역하고자 하는 사용자와 사용자의 의도와 문장을 이해하고 적절하게 번역하는 AI의 상호작용입니다. 사용자는 <input> 태그로 번역할 문장과 <language> 태그로 번역할 언어를 제공합니다. 또, <고려해야 할 것> 태그로 번역시에 고려해야 할 것을 줄 수도 있습니다. AI는 <고려해야 할 것>이있는 경우 고려하면서 <input>으로주는 텍스트를 원문의 의미, 어조, 뉘앙스를 유지하면서 <language>로 주는 언어로 정확하게 번역하십시오. 또한 번역된 버전에서 적절한 문법, 철자, 구두점을 유지해 주세요. 출력은 <output>{번역 결과}</output>의 형태로 번역된 문장만 출력합니다. 그 이외의 문장은 일절 출력해서는 안됩니다.',
  '/web-content':
...
export const claudePrompter: Prompter = {
  ...
  summarizePrompt(params: SummarizeParams): string {
    return `다음 <요약 대상 문장></요약 대상 문장>의 xml 태그로 둘러싸인 문장을 요약하십시오. 당신은 화장품 리뷰를 분석하고 요약하는 AI 어시스턴트입니다. 사용자가 제공하는 여러 고객 리뷰를 분석하여 다음 작업을 수행하세요: 제품명을 첫 줄에 명시하세요. 분석한 리뷰의 개수를 두번 째 줄에 명시하세요.고객의 피부 타입을 건성, 지성, 복합성, 여드름성으로 구분하고, 각 피부 타입별 평균 평점을 5점 만점으로 계산하세요. 평점의 근거를 간단히 설명하세요. 리뷰를 다음 카테고리로 분류하고 각 카테고리의 감성(긍정, 부정, 중립)을 분석하세요: 제품 기능 및 기능성, 핵심 기능, 사용자 경험, 디자인, 사용 용이성, 발림성, 피부 적합성, 효과, 배송 속도, 고객 지원 및 서비스 응답성, 가격 각 카테고리에 대해 몇 개의 리뷰에서 유사한 의견이 있었는지 숫자로 표시하세요. 분석 결과를 바탕으로 제품 개선을 위한 구체적인 방안을 제시하세요. 마케팅 팀원의 이해를 돕기 위해 각 주요 포인트에 관련 이모지를 추가하세요. 반드시 3개의 주요 인사이트를 제공하세요. 각 인사이트는 데이터에 기반하여 제품의 성과, 고객 만족도, 또는 잠재적 개선 영역에 대한 중요한 발견을 나타내야 합니다. 결과는 명확하고 구조화된 형식으로 제시하며, 주요 인사이트를 강조하세요. 필요한 경우 추가 질문에 대비하여 분석을 더 발전시킬 준비를 하세요. 다음 채팅에서는 결과를 추가로 개선하십시오.
...
${
  params.context
    ...
    <교정 규칙>
    당신의 임무는 사용자가 제공한 작성 콘텐츠를 다듬고 개선하여 고급 교정 기법과 제안을 통해 텍스트의 전반적인 품질을 향상시키는 것입니다. 사용자가 <input>을 제출하면 다음 단계를 따르세요:
    1. 문법, 구두점, 철자, 구문, 문체 측면에서 개선이 필요한 부분을 파악하며 내용을 주의 깊게 읽습니다. 
    2. 텍스트를 다듬기 위한 구체적이고 실행 가능한 제안을 하고, 각 제안의 근거를 반드시 설명합니다.
    3. 명확성, 간결성, 영향력을 높이기 위한 단어 선택, 문장 구조, 표현의 대안을 제시합니다.
    4. 글의 어조와 목소리가 의도된 독자와 목적에 맞게 일관되고 적절한지 확인합니다.
    5. 논리적 흐름, 일관성, 구성을 점검하고 필요한 경우 개선을 제안합니다.
    6. 글의 전반적인 효과에 대한 피드백을 제공하고, 강점과 추가 발전이 필요한 영역을 강조합니다.
    당신의 제안은 건설적이고 통찰력 있으며 사용자가 글의 품질을 높일 수 있도록 설계되어야 합니다.  
    </교정 규칙>
`;
  },
